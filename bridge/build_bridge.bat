...
REM The bridge is self-contained: outlook_bridge.py imports outlook_ops.py
REM from THIS folder (no ..\src dependency), so PyInstaller bundles it
REM automatically. Only external COM packages need hidden imports.
pyinstaller --onefile --name outlook_bridge --hidden-import win32com --hidden-import win32com.client --hidden-import pythoncom --hidden-import pywintypes --collect-submodules win32com outlook_bridge.py
if errorlevel 1 goto :fail
